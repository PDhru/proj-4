...
app.post('/addbook', async (req, res) => {
  ...
  try {
      const newBook = new bookData({bookName,bookAuthor,bookUrl,bookDisc,bookPrice});
      await newBook.save();
      res.redirect('/');
  } catch (err) {
...
app.post('/updatebook/:id', async (req, res) => {
  const { bookName, bookAuthor, bookUrl, bookDisc, bookPrice } = req.body;
  try {
      await bookData.findByIdAndUpdate(req.params.id, {bookName,bookAuthor,bookUrl,bookDisc,bookPrice});
      res.redirect('/');
  } catch (err) {
